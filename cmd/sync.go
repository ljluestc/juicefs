 ...
 */
 ...
 func cmdSync() *cli.Command {
	 return &cli.Command{
		 ...
		 ArgsUsage: "SRC DST",
		 Description: `
 This tool spawns multiple threads to concurrently sync objects of two data storages.
 ...
 func doSync(c *cli.Context) error {
	 ...
	 go func() {
		 <-sigChan
		 logger.Infof("Received interrupt signal, cleaning up temporary files...")
		 sync.CleanupTempFiles(dst, config)
		 os.Exit(1)
	 }()
